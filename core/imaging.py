...
# Discrete Sharpen strength → (bilateral_d, sigma_color, sigma_space, blur_sigma). Mirrors
# _STRENGTH's role for B/W: higher strength = MORE denoise before a harder unsharp (amount from
# CLEAN_AMOUNT). Fixes the bug where only `amount` scaled, so strength 3 amplified scan noise
# (hard sharpen over fixed light denoise) and strength 1 over-smoothed.
_GRAY_STRENGTH = {
    1: dict(d=5, sigma_color=25, sigma_space=25, blur_sigma=1.2),   # cautious
    2: dict(d=5, sigma_color=40, sigma_space=40, blur_sigma=2.0),   # normal (former fixed values)
    3: dict(d=7, sigma_color=60, sigma_space=60, blur_sigma=2.8),   # aggressive
}


# --------------------------------------------------------------------------- onnx
class Int64Session:
    """A thin proxy around an ONNX `InferenceSession` that up-casts int32 input tensors to
    int64 before `run()` (docuwarp's grid graph requires int64; some exports emit int32).

    We wrap the session rather than monkey-patching its bound `run` method: the real session
    object is left untouched, and every other attribute/method is delegated unchanged — far
...
def clean_document_bilevel(
    ...
    *,
    ...
    if upscale and upscale != 1.0:
        ...
        return np.where(out < 128, 0, 255).astype(np.uint8)
    return hi
...
def unwarp_bgr(bgr: NDArr, *, providers: Tuple[str, ...] = ("CPUExecutionProvider",)) -> NDArr:
    ...
    if "u" not in _UNWARP_CACHE:
        ...
        u.bilinear_unwarping = Int64Session(u.bilinear_unwarping)   # int32→int64 grid inputs
        _UNWARP_CACHE["u"] = u
